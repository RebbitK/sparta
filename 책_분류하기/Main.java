...
public class Main {
    public static void main(String[] args) {
        ...
        bookList.stream().filter(book -> book.getCategory()=="여행").forEach(book -> System.out.println("카테고리가 여행인 책: "+book.getBookName()));
        bookList.stream().filter(book -> book.getPrice()<=16200).forEach(book -> System.out.println("가격 16200원 이하인 책: "+book.getBookName()));
        bookList.stream().filter(book -> book.getBookName().contains("경제")).forEach(book -> System.out.println("책 제목에 경제가 들어간 책: "+book.getBookName()));
        double maxPrice = bookList.stream().mapToDouble(Book::getPrice).max().getAsDouble();
        System.out.println("책 목록 중 가장 비싼 금액: " + maxPrice);
        System.out.println();
        double sum = bookList.stream().filter(book -> book.getCategory()=="IT").mapToDouble(Book::getPrice).sum();
        System.out.println("카테고리 IT 책들의 가격 합: " + sum);
        System.out.println();
        List<Book> discountedBookList = bookList.stream().filter(book -> book.getCategory()=="IT").map(book -> {book.setPrice(book.getPrice()*0.6); return book;}).toList();
